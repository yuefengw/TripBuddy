...
class RagAgentService:
    ...
    def __init__(self, streaming: bool = True):
        ...
        # 使用 SkillRegistry 统一管理工具
        # 基础工具（由 SkillRegistry 初始化时提供）
        self.tools: list = []
    ...
    async def _initialize_agent(self):
        """异步初始化 Agent（通过 SkillRegistry 获取所有工具）"""
        if self._agent_initialized:
            return

        # 通过 SkillRegistry 获取所有已启用的工具
        self.tools = skill_registry.get_all_tools()
        logger.info(f"通过 SkillRegistry 获取了 {len(self.tools)} 个工具")
