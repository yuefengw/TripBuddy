...
async def get_tools_by_server(server_name: str) -> List[Any]:
    """获取指定 MCP 服务器的工具

    Args:
        server_name: 服务器名称（如 "cls", "monitor"）

    Returns:
        List[Any]: 该服务器提供的工具列表
    """
    client = await get_mcp_client_with_retry()
    tools = await client.get_tools()

    # 如果是 MultiServerMCPClient，可以按服务器过滤工具
    # 工具的 name 格式通常是 "server_name_toolname"
    filtered_tools = []
    for tool in tools:
        tool_name = getattr(tool, "name", str(tool))
        # 简单过滤：实际使用时 langchain-mcp-adapters 会处理
        filtered_tools.append(tool)

    logger.info(f"从服务器 '{server_name}' 获取了 {len(filtered_tools)} 个工具")
